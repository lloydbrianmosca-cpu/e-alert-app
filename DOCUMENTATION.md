...
│                                                                  │
│  ┌─────────────┐   ┌─────────────┐   ┌─────────────────────┐   │
│  │   User App  │   │ Responder   │   │    Admin Panel      │   │
│  │             │   │    App      │   │                     │   │
│  └──────┬──────┘   └──────┬──────┘   └──────────┬──────────┘   │
│         │                 │                      │              │
│         └─────────────────┼──────────────────────┘              │
...
│   └── EmergencyProvider (Emergency State)
...
│   │   ├── 📄 AuthHeader.js  # Authentication header
...
│   │   ├── 📄 addressData.js # Philippine address data (Regions, Provinces, Cities)
│   │   └── 📄 index.js       # Constants exports
│   │
│   ├── 📁 context/           # React Context providers
│   │   ├── 📄 AuthContext.js     # Authentication state management
│   │   ├── 📄 ChatContext.js     # Chat/messaging state management
│   │   └── 📄 EmergencyContext.js # Emergency state & algorithms
│   │
...
│   │   ├── 📄 HomeScreen.js           # User dashboard + SOS
│   │   ├── 📄 LocationsScreen.js      # Map view with tracking
│   │   ├── 📄 ChatScreen.js           # Real-time messaging
│   │   ├── 📄 HotlinesScreen.js       # Emergency contacts directory
│   │   ├── 📄 ProfileScreen.js        # User profile management
│   │   ├── 📄 AdminHomeScreen.js      # Admin dashboard
│   │   ├── 📄 ResponderHomeScreen.js  # Responder dashboard
│   │   ├── 📄 ResponderLocationsScreen.js # Responder map view
│   │   ├── 📄 ResponderChatsScreen.js # Responder messaging
│   │   ├── 📄 ResponderProfileScreen.js # Responder profile
│   │   ├── 📄 ResponderEmergencyHistoryScreen.js # Response history
│   │   ├── 📄 ResponderSignUpScreen.js # Register new responders
│   │   ├── 📄 ResponderManagementScreen.js # Manage responders
│   │   ├── 📄 UserLogsScreen.js       # User activity logs
│   │   ├── 📄 EmergencyHistoryScreen.js # All emergency history
│   │   ├── 📄 RealtimeMonitoringScreen.js # Live emergency monitoring
│   │   └── 📄 index.js       # Screen exports
│   │
│   └── 📁 services/          # Firebase services
│       ├── 📄 firebase.js    # Firebase app initialization
│       ├── 📄 firestore.js   # Database operations (CRUD, queries)
│       └── 📄 storage.js     # File storage operations (images)
│
...
SignIn ─┬─► SignUp
        ├─► ForgotPassword
        │
        ├─► Home (User) ─┬─► Locations (Map)
        │                ├─► Hotlines
        │                ├─► Chat
        │                └─► Profile
        │
        ├─► ResponderHome ─┬─► ResponderLocations
        │                  ├─► ResponderChats
        │                  ├─► ResponderHistory
        │                  └─► ResponderProfile
        │
        └─► AdminHome ─┬─► ResponderSignUp
                       ├─► UserLogs
                       ├─► EmergencyHistory
                       ├─► RealtimeMonitoring
                       └─► ResponderManagement
...
│       ├── lastName: string
│       ├── email: string
│       ├── responderType: string ('police' | 'medical' | 'fireman' | 'flood')
│       ├── stationName: string
...
│       ├── userName: string
│       ├── userLocation: object
│       ├── completedBy: string (responderId)
...
└── 📁 conversations/
    └── {conversationId}
        ...
        ├── responderBuilding: string
        ├── emergencyType: string
        ├── emergencyId: string
        ├── lastMessage: string
        ├── lastMessageAt: timestamp
        ├── userUnread: number
        ├── responderUnread: number
        ├── status: string ('active' | 'closed')
        ├── expiresAt: timestamp (3 days)
        ├── createdAt: timestamp
        │
        └── 📁 messages/
            └── {messageId}
                ├── text: string
                ├── senderId: string
                ├── senderType: string ('user' | 'responder')
                ├── timestamp: timestamp
                └── read: boolean
```
...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    match /responders/{responderId} {
      // Anyone authenticated can read responders (needed for finding nearest)
      allow read: if request.auth != null;
      // Only the responder themselves can write
      allow write: if request.auth != null && request.auth.uid == responderId;
      // Admins can write to any responder
      allow write: if request.auth != null && 
                     get(/databases/$(database)/documents/users/$(request.auth.uid)).data.role == 'admin';
    }
    
    // Active emergencies - authenticated users only
    match /activeEmergencies/{emergencyId} {
      allow read, write: if request.auth != null;
    }
    
    // Emergency history - authenticated users only
  ...
  "indexes": [
    {
      ...
      "fields": [
        { "fieldPath": "responderType", "order": "ASCENDING" },
        { "fieldPath": "isAvailable", "order": "ASCENDING" }
      ]
    },
    {
      "collectionGroup": "activeEmergencies",
      "queryScope": "COLLECTION",
      "fields": [
        { "fieldPath": "assignedResponderId", "order": "ASCENDING" },
        { "fieldPath": "createdAt", "order": "DESCENDING" }
      ]
    },
    {
      "collectionGroup": "emergencyHistory",
      "queryScope": "COLLECTION",
      "fields": [
...
| Type | Color Name | Hex Code | RGB | Usage |
|------|------------|----------|-----|-------|
| 🚔 Police | Navy Blue | `#1E3A8A` | rgb(30, 58, 138) | Police emergencies |
| 🚒 Fire | Crimson Red | `#DC2626` | rgb(220, 38, 38) | Fire emergencies |
| 🏥 Medical | Emerald Green | `#059669` | rgb(5, 150, 105) | Medical emergencies |
| 🌊 Flood | Ocean Blue | `#0369A1` | rgb(3, 105, 161) | Flood/Rescue emergencies |

### UI Colors

| Element | Color Name | Hex Code | Usage |
|---------|------------|----------|-------|
| Primary | Red | `#DC2626` | Primary buttons, headers |
...
| Type | Light Color | Hex Code |
|------|-------------|----------|
| Police | Light Blue | `#DBEAFE` |
| Fire | Light Red | `#FEE2E2` |
| Medical | Light Green | `#D1FAE5` |
| Flood | Light Cyan | `#E0F2FE` |
