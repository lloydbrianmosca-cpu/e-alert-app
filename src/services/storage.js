 ...
 * @param {object} imageAsset - Image asset from ImagePicker
 */
export const uploadValidID = async (userId, imageAsset) => {
  try {
    if (!imageAsset || !imageAsset.uri) {
      throw new Error('Invalid image asset');
    }

    console.log('Starting valid ID upload...', { userId, uri: imageAsset.uri });

    // Create a reference to the file
    const fileName = `user/${userId}/valid-id-${Date.now()}.jpg`;
    const storageRef = ref(storage, fileName);
    ...
    console.log('Blob created, uploading valid ID to Firebase...');

    // Upload the file
    const snapshot = await uploadBytes(storageRef, blob);
    console.log('Valid ID upload complete, getting download URL...');

    // Get the download URL
    const downloadURL = await getDownloadURL(snapshot.ref);
    console.log('Valid ID download URL:', downloadURL);

    return downloadURL;
  } catch (error) {
    console.log('Error uploading valid ID:', error);
    throw error;
  }
};

/**
 * Delete valid ID image from Firebase Storage
 * @param {string} imageUrl - The Firebase Storage URL of the image
 */
export const deleteValidID = async (imageUrl) => {
  try {
    if (!imageUrl) return;
    ...
    console.log('Valid ID deleted successfully');
  } catch (error) {
    console.log('Error deleting valid ID:', error);
    // Don't throw - deletion errors shouldn't break the app
  }
};
